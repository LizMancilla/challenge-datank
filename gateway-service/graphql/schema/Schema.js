...
const getPoll = (id) => {
  const url = 'http://'+process.env.POLL_HOST+':3300/v1/poll/verPoll'
  return fetch(url)
    .then(response => {
...
const statPoll = (id) => {
  const url = 'http://'+process.env.STATS_HOST+':4400/v1/stats/statPoll'
  return fetch(url)
    .then(response => {
      return response.json()
    })
...
const addPoll = (name, options) => {
  const url = 'http://'+process.env.POLL_HOST+':3300/v1/poll/addPoll?name=' + name + '&opts=' + options
  return fetch(url)
    .then(response => {
      return getPoll(0)
    })
...
const vote = (id, option) => {
  const url = 'http://'+process.env.STATS_HOST+':4400/v1/stats/vote?id=' + id + '&opt=' + option
  return fetch(url)
    .then(response => {
      return response.json()
    })
